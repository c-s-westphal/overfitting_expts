...
def main():
    parser = argparse.ArgumentParser(description='Single run for varying dataset size experiment')
    ...
    trainloader, testloader = get_cifar10_dataloaders(
        ...
        num_workers=4,
        subset_size=args.dataset_size if args.dataset_size < 60000 else None,
        seed=args.seed
    )
    ...
    if metrics['final_train_acc'] >= 99.0:
        ...
        save_path = f"{args.output_dir}/{args.model}_size{args.dataset_size}_seed{args.seed}_results.npz"
