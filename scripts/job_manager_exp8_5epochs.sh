...
number=$SGE_TASK_ID
paramfile="scripts/jobs_exp8_5epochs.txt"
...
# ---------------------------------------------------------------------
# Format per line in jobs_exp8_5epochs.txt:
#   <depth> <seed>
...
date
echo "Running exp8 Variable-Depth MLP on CelebA (5 epochs): depth=$depth, seed=$seed"
...
python3.9 -u experiments/exp8_single_run.py \
    --depth "$depth" \
    --seed "$seed" \
    --epochs 5 \
    --batch_size 128 \
...
date
echo "Training completed: MLP depth=$depth seed=$seed (5 epochs)"
