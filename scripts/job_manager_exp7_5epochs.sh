...
number=$SGE_TASK_ID
paramfile="scripts/jobs_exp7_5epochs.txt"
...
# ---------------------------------------------------------------------
# Format per line in jobs_exp7_5epochs.txt:
#   <arch> <seed>
...
date
echo "Running exp7 Full-Depth VGG on CelebA (5 epochs): arch=$arch, seed=$seed"
...
python3.9 -u experiments/exp7_single_run.py \
    --arch "$arch" \
    --seed "$seed" \
    --epochs 5 \
    --batch_size 128 \
